...
/**
 * From contribution codes (soooooo much time saved !)<br/>
...
class VadsWSApi {
	...
	function initialize($key, $wsdl) {
		...
		$this->_addField( 'shopId', 'Identifiant de la boutique', true );
		...
		$this->_addNumberField( 'sequenceNb', 'Numéro de séquence de la transaction', true );
		...
		//FIXME use SoapClient classmap option to simplify all that !
	...
	/**
	 * Public static method to compute the signature.
	 * The parameters list to use is
	...
	function getSignature($fieldSet) {
		...
		$fields = $this->$fieldSet;
		foreach ( $fields as $field ) {
			$value = array_key_exists( $field, $this->wsParams ) ? $this->wsParams [$field]->getValue4Sign() : null;
			if ($value == null) {
				$value = '';
			}
			$raw_sign .= $value . '+';
	...
	/**
	 * Public static method to prepare data for ws call.
	 * The parameters list to use is
	...
	function _getResponseType($method_name) {
		if ($method_name == self::METHOD_REFUND || $method_name == self::METHOD_GET_INFO || $method_name == self::METHOD_DUPLICATE || $method_name == self::METHOD_CREATE) {
